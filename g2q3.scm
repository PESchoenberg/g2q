...
;
(define (compile-and-run p_fname p_fnameo p_qver p_ddir p_qpu p_qf p_q p_c p_qn p_cn p_mc p_i)
  (let ((porto1 (current-output-port))
    ...
    (set! c (strings-append (list "./qre" p_fnameo "post" "n" p_qpu "1" fget) 1))
    (system c)
...
; p_qx: qx
;
(define (main-loop p_clean p_fname p_fnameo p_qver p_ddir p_qpu p_qf p_q p_c p_qn p_cn p_mc p_qx)
  (let ((res 0))    
    (let loop ((i p_qx))
      (if (= i 0)
	  (begin (if(equal? p_clean "y")(system (strings-append (list "rm " p_ddir p_fname "_*") 0)))
	  res)
	  (begin (set! res (+ res (compile-and-run p_fname p_fnameo p_qver p_ddir p_qpu p_qf p_q p_c p_qn p_cn p_mc i)))
		 (loop (- i 1)))))))
